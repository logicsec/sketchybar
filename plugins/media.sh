...
def fmt_msg(msg, length):
    return f"{msg:.{length - 1}}…" if len(msg) > length else f"{msg:<{length}}"
...
def main():
    FIXED_WIDTH = 20  # Define a constant for the fixed width
    ...
    print(fmt_msg(now_playing, FIXED_WIDTH))
    sleep(2)

    marquee = f"{now_playing}     {now_playing}"
    marquee_len = FIXED_WIDTH
    ...
    for i in range(len(now_playing) + 7):
        ...
        # Ensure display is exactly FIXED_WIDTH characters
        display = f"{display:<{FIXED_WIDTH}}"
